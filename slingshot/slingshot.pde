...
PVector           penBall                             = new PVector(0, 0);
PVector           posWall                             = new PVector(0.01, 0.05);
PVector           posBall                             = new PVector(0.13,0.07) ;
...
void setup(){
  ...
  balloon = createShape(ELLIPSE, 500, 250, 24, 24);
  print(posWall.x);
  balloon.setStroke(color(0));
...
PShape create_balloon(float x1, float y1, float x2, float y2){
  x1 = pixelsPerMeter * x1;
  y1 = pixelsPerMeter * y1;
  x2 = pixelsPerMeter * x2;
  y2 = pixelsPerMeter * y2;
  
  return createShape(ELLIPSE, deviceOrigin.x + x1, deviceOrigin.y + y1, deviceOrigin.x + x2, deviceOrigin.y+y2);
}
